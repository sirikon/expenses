import { Credentials, login } from "@/sources/bbva.ts";
...
async function askCredentials(): Promise<Credentials> {
  ...
  if (processStatus.code > 0) {
    throw new Error(
      `Asking credentials failed with exit code ${processStatus.code}`,
    );
  }
  const [username, password] =
    (new TextDecoder().decode(await process.output())).trim().split("|");
  return {
    username,
    password,
  };
